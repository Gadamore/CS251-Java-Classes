...
public class Lab12  {
	...
	// salary choice
	//private JComboBox<String> salary; // NEED TO UPDATE TO ECLIPSE 8
	...
	JPanel addPositions() { 
		...
		radioGroup.add(softwareEngineer);
		radioGroup.add(systemEngineer);
		
		softwareEngineer.setSelected(true);
		
		position.add(softwareEngineer);
		position.add(systemEngineer);
		
		ActionListener radioButton = new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				Object src = e.getSource();
				if (src == softwareEngineer){
					centerPanel.remove(systemPanel);
					centerPanel.add(softwarePanel, 1);
					centerPanel.validate();
					centerPanel.repaint();
				}
				else {
					centerPanel.remove(softwarePanel);
					centerPanel.add(systemPanel, 1);
					centerPanel.validate();
					centerPanel.repaint();
				}
		...
		softwareEngineer.addActionListener(radioButton);
		systemEngineer.addActionListener(radioButton);
